...
$Usuarios = $acao!='NEW' ? User::obterUsuarios($id) : array();
				...
				<div class="card-footer">
					<div class="form-group col-12">
						<div class="row">
							...
							<?php
							  if($acao=='EDIT'){ ?>
								<button class="btn btn-success mr-1" name="submit" onclick="saveUsuario(this);">Atualizar</button>
						<?php  }
							  if($acao=='NEW'){ ?>
								<button class="btn btn-danger ml-1" name="submit" onclick="saveUsuario(this)">Cadastrar</button>
						<?php  } ?>
							</div>
						</div>
					</div>
...
<script>
	...
	function saveUsuario(e){
		event.preventDefault();
			let form = e.closest('form') ? e.closest('form') : document, o = Utils.serial2Obj($(form).serialize()), token = Utils.getToken();
			if(Object.keys(o).length>0){
				if(Utils.isCPF(o.login)||Utils.isCNPJ(o.login)){
					return fetch(`./../assets/classes/User.php?a=saveUsuario&token=${token}`,{
						method: 'POST', headers: {
